...
Please enable it in the DocPilot panel settings.`);
      ...
      if (enabledSources.length === 0) {
        stream.markdown(`📚 **No documentation sources configured**
...
Please add and enable documentation sources in the DocPilot panel first.`);
      ...
      if (retrievalResult.chunks.length === 0) {
        stream.markdown(`❌ **No relevant documentation found**
...
Please open a workspace folder first.`);
      ...
      try {
        ...
        stream.markdown(`✅ **Context saved to file**
...
*Click the file link above to open it, then copy the contents to attach to your Copilot chat.*`);
        ...
        stream.markdown(`❌ **Failed to save file:** ${error instanceof Error ? error.message : String(error)}`);
...
\`\`\``);
      ...
      stream.markdown(`❌ **Error generating context:** ${message}`);
